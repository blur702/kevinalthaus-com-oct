...
This guide provides essential knowledge for AI coding agents working with this microservices platform.

## Project Overview

**Microservices Platform** built with Node.js 20+, TypeScript, React, and PostgreSQL 16. Emphasizes security (RBAC, JWT auth, CSRF protection), extensibility (plugin system), and scalability (independent service scaling).
...
**Critical:** In production, only API Gateway, Frontend, and Admin are exposed to the host. All backend services are accessible only within the Docker network `app_network`.
...
**Core shared library** imported by all services. Key exports:
...
3. Ensure capabilities are granted in admin UI</content>
<parameter name="filePath">e:\dev\kevinalthaus-com-oct\docs\ai-coding-guide.md