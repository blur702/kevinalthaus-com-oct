...
class PageService {
    ...
    async createPage(input) {
        ...
        // Validate layout
        const validatedLayout = (0, types_1.validatePageLayout)(input.layout_json);
        // Sanitize inputs
        const sanitizedTitle = (0, sanitize_html_1.default)(input.title, { allowedTags: [] });
        const sanitizedMetaDesc = input.meta_description
            ? (0, sanitize_html_1.default)(input.meta_description, { allowedTags: [] })
            : null;
        const result = await this.pool.query(`INSERT INTO plugin_page_builder.pages
    ...
    async getPageBySlug(slug) {
        const result = await this.pool.query('SELECT * FROM plugin_page_builder.pages WHERE slug = $1 AND deleted_at IS NULL', [slug]);
        if (result.rows.length === 0) {
            return null;
        }
        return this.mapRowToPage(result.rows[0]);
    ...
    async listPages(options = {}) {
        ...
        if (search) {
            whereConditions.push(`(title ILIKE $${paramIndex} OR slug ILIKE $${paramIndex} OR meta_description ILIKE $${paramIndex})`);
            params.push(`%${search}%`);
            paramIndex++;
        }
        const whereClause = whereConditions.join(' AND ');
        // Get total count
        const countResult = await this.pool.query(`SELECT COUNT(*) FROM plugin_page_builder.pages WHERE ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].count);
        // Get pages
        const result = await this.pool.query(`SELECT * FROM plugin_page_builder.pages
       WHERE ${whereClause}
    ...
    async updatePage(id, input) {
        ...
        if (input.title !== undefined) {
            ...
            params.push((0, sanitize_html_1.default)(input.title, { allowedTags: [] }));
            paramIndex++;
        }
        ...
        if (input.meta_description !== undefined) {
            ...
            params.push(input.meta_description ? (0, sanitize_html_1.default)(input.meta_description, { allowedTags: [] }) : null);
            paramIndex++;
        }
        ...
        paramIndex++;
        params.push(id);
        const result = await this.pool.query(`UPDATE plugin_page_builder.pages
       SET ${updates.join(', ')}
     ...
     * Soft delete page
     */
    async deletePage(id, deleted_by) {
        const result = await this.pool.query(`UPDATE plugin_page_builder.pages
       SET deleted_at = CURRENT_TIMESTAMP, deleted_by = $1
       WHERE id = $2 AND deleted_at IS NULL
       RETURNING id`, [deleted_by, id]);
    ...
    async getPageVersions(pageId) {
        const result = await this.pool.query(`SELECT * FROM plugin_page_builder.page_versions
       WHERE page_id = $1
    ...
    async getPageVersion(pageId, versionNumber) {
        const result = await this.pool.query(`SELECT * FROM plugin_page_builder.page_versions
       WHERE page_id = $1 AND version_number = $2`, [pageId, versionNumber]);
        if (result.rows.length === 0) {
            return null;
        }
    ...
    async createTemplate(input) {
        const id = (0, uuid_1.v4)();
        const validatedLayout = (0, types_1.validatePageLayout)(input.layout_json);
        const result = await this.pool.query(`INSERT INTO plugin_page_builder.templates
       (id, name, description, thumbnail_url, layout_json, category, is_public, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`, [
            id,
            (0, sanitize_html_1.default)(input.name, { allowedTags: [] }),
     ...
     * List templates
     */
    async listTemplates(userId, category) {
        let query = `
    ...
    async createReusableBlock(input) {
        const id = (0, uuid_1.v4)();
        const result = await this.pool.query(`INSERT INTO plugin_page_builder.reusable_blocks
     ...
     */
    async listReusableBlocks(userId, category) {
        let query = `
