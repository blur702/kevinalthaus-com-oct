...
import { createSSDDRouter } from './routes';

export default class SSDDValidatorPlugin implements PluginLifecycleHooks {
  ...
  async onInstall(context: PluginExecutionContext): Promise<void> {
    ...
    this.logger.info('Installing SSDD Validator Plugin...');

    try {
      await this.runMigrations(context);
      this.logger.info('SSDD Validator Plugin installed successfully');
    } catch (error) {
      this.logger.error('Failed to install SSDD Validator Plugin', error as Error);
      throw error;
    }
  }
  ...
  async onActivate(context: PluginExecutionContext): Promise<void> {
    ...
    this.logger.info('Activating SSDD Validator Plugin...');

    try {
      // Register API routes
      if (context.app && context.db && context.services) {
        const router = createSSDDRouter(context);
        context.app.use('/api/ssdd-validator', router);

        this.logger.info('Routes registered successfully');
      } else {
        throw new Error('Required context properties not available (app, db, or services)');
      }

      this.logger.info('SSDD Validator Plugin activated successfully');
    } catch (error) {
      this.logger.error('Failed to activate SSDD Validator Plugin', error as Error);
      throw error;
    }
  }
  ...
  async onDeactivate(context: PluginExecutionContext): Promise<void> {
    ...
    this.logger.info('Deactivating SSDD Validator Plugin...');

    try {
      this.logger.info('SSDD Validator Plugin deactivated successfully');
    } catch (error) {
      this.logger.error('Failed to deactivate SSDD Validator Plugin', error as Error);
      throw error;
    }
  }
  ...
  async onUninstall(context: PluginExecutionContext): Promise<void> {
    ...
    this.logger.info('Uninstalling SSDD Validator Plugin...');

    // WARNING: Uncomment this to delete all plugin data
    // await this.pool.query('DROP SCHEMA IF EXISTS plugin_ssdd_validator CASCADE');

    this.logger.info('SSDD Validator Plugin uninstalled (data preserved for safety)');
  }
  ...
  async onUpdate(context: PluginExecutionContext, oldVersion: string): Promise<void> {
    ...
    this.logger.info(`Updating SSDD Validator Plugin from version ${oldVersion}...`);

    try {
      await this.runMigrations(context);
      this.logger.info('SSDD Validator Plugin updated successfully');
    } catch (error) {
      this.logger.error('Failed to update SSDD Validator Plugin', error as Error);
      throw error;
    }
  }
  ...
  private async runMigrations(_context: PluginExecutionContext): Promise<void> {
    ...
    try {
      const result = await this.pool.query<{ migration_name: string }>(
        `SELECT migration_name FROM plugin_ssdd_validator.plugin_migrations ORDER BY executed_at`
      );
      ...
      for (const file of migrationFiles) {
        ...
        try {
          ...
          await client.query(
            `INSERT INTO plugin_ssdd_validator.plugin_migrations (migration_name, executed_at)
